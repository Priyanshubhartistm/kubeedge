...
func TestProcessFilter_ShortCircuitsOnError(t *testing.T) {
	...
	assert.ErrorIs(t, err, filterErr, "expected the filter error to be returned")
...
// returns an error the remaining filters are skipped and the exact error is
// returned.
...
// TestProcessFilter_PassesMessageToAllFilters verifies that the same
// *model.Message pointer is passed to every filter function unchanged.
func TestProcessFilter_PassesMessageToAllFilters(t *testing.T) {
	msg := newMsg("m7")
